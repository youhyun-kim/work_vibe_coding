...
export const MeetingHistoryView: React.FC<MeetingHistoryViewProps> = ({
  ...
  // Combine custom & mock meetings, sorted by date (newest first)
  const allMeetings = [...customMeetings, ...MOCK_MEETINGS];
  const [selectedMeeting, setSelectedMeeting] = useState<MeetingRecord | null>(allMeetings[0]);
  ...
  const [purgedMeetingIds, setPurgedMeetingIds] = useState<Set<string>>(
    new Set(['mtg_002'])
  );
